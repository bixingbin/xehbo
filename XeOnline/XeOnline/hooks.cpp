		...
		namespace titles {
			...
			VOID initialize(PLDR_DATA_TABLE_ENTRY ModuleHandle)
			{
				...
				else if (pExecutionId->TitleID == COD_BO3)
				{
					...
					if (wcscmp(ModuleHandle->BaseDllName.Buffer, L"default.xex") == 0)
					{
						xbox::utilities::setMemory((LPVOID)mpPatch1BO3, 0x39600000);
						xbox::utilities::setMemory((LPVOID)mpPatch2BO3, 0x48000010);
						xbox::utilities::setMemory((LPVOID)mpPatch3BO3, 0x38600000);
						xbox::utilities::setMemory((LPVOID)mpPatch4BO3, 0x39600001);
					}
					else if (wcscmp(ModuleHandle->BaseDllName.Buffer, L"default_zm.xex") == 0)
					{
						xbox::utilities::setMemory((LPVOID)zmPatch1BO3, 0x48000010);
						xbox::utilities::setMemory((LPVOID)zmPatch2BO3, 0x60000000);
						xbox::utilities::setMemory((LPVOID)zmPatch3BO3, 0x39600001);
					}
				}
			}
		}
